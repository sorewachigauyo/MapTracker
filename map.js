...
let idx = 0;
//1: doppelsodner, 2: uhlan, 3: rodel, 4: nyto
const enemy_map = {
  0: 91086,
  1: 91082,
  2: 91087,
...
class Map {
    ...
    constructor ({
      ...
      this.enemy_team_info = enemy_team_info,
      this.enemy_character_type_info = enemy_character_type_info
      // init Object data
        ...
        if (lastX == x && lastY == y) {
          ...
          $.each(this.mission_info[this.missionId].spot_ids, (index, spot_id) => {
            ...
            if (Math.abs(spot.coordinator_x - x) <= w / 2 && Math.abs(spot.coordinator_y - y) <= h / 2) {
              // Remove on right click, add on left click
              if (evt.button === 2) {
                spot.enemy_team_id = 0;
              }
              if (evt.button === 0) {
                spot.enemy_team_id = enemy_map[idx];
              }
              this.drawFgImage(canvas);
    ...
    drawBuilding (ctx, x0, y0, building) {
      ...
      if (spineImg != null) {
        ...
        this.drawBgSpineAlternativeText(ctx, x0, y0, (building.name));
      }
    }
    ...
    drawBgSpineAlternativeText (ctx, x0, y0, text, selected) {
      var scale = this.scale;
      ctx.save();
      ctx.font = "bold " + Math.floor(32 / scale) + "px " + ("font.sans-serif");
    ...
    drawFriendStats (ctx, x0, y0, name, nameColor, order, orderColor, power, hp, hpType, hpColor) {
      ...
      ctx.textAlign = "start";
      ctx.textBaseline = "middle";
      ctx.font = "bold " + Math.floor(36 * scale) + "px " + ("font.serif");
      ...
      if (order != "") {
        ...
        ctx.fill();
        ctx.font = Math.floor(36 * scale) + "px " + ("font.sans-serif");
        ctx.textAlign = "center";
        ctx.fillStyle = "black";
